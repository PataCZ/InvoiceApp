...
public class PersonServiceImpl implements PersonService {
    ...
    public PersonDTO updatePerson(long personId, PersonDTO personDTO) {
        ...
        if(!existingPerson.getIdentificationNumber().equals(personDTO.getIdentificationNumber())) {
            throw new IllegalArgumentException("Cannot change IČO");
        }
        existingPerson.setHidden(true);
        personRepository.save(existingPerson);

        PersonEntity newPersonEntity = personMapper.toEntity(personDTO);
        newPersonEntity.setId(personId);
        newPersonEntity.setIdentificationNumber(existingPerson.getIdentificationNumber());
        newPersonEntity.setHidden(false);
